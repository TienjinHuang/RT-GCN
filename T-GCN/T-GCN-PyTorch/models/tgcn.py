...
class RTGCNGraphConvolution(nn.Module):
    ...
    def forward(self, inputs, hidden_state):
        ...
        concatenation_mean = concatenation_mean @ self.weights_mean + self.biases_mean
        concatenation_var = concatenation_var @ self.weights_var + self.biases_var
...
class TGCNCell(nn.Module):
    def __init__(self, adj, input_dim: int, hidden_dim: int,lamda=1.0):
    ...
    def forward(self, inputs, hidden_state):
        ...
        c = self.graph_conv2(inputs, r * hidden_state)

        c_mean,c_var=torch.chunk(c,chunks=2,dim=-1)
        c_mean=torch.tanh(c_mean)
...
class TGCN(nn.Module):
    def __init__(self, adj, hidden_dim: int,lamda:float, **kwargs):
        super(TGCN, self).__init__()
        self._input_dim = adj.shape[0]
        self._hidden_dim = hidden_dim
        self.register_buffer("adj", torch.FloatTensor(adj))
        self.tgcn_cell = TGCNCell(self.adj, self._input_dim, self._hidden_dim,lamda)
